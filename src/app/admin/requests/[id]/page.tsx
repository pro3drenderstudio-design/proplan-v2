...
// Add updateProjectStatus to admin-api if missing — we'll cast to any for now
...
function buildTimeline(project: Project) {
  ...
  return TIMELINE
    ...
    .map((stage, i) => ({
      ...stage,
      timestamp: new Date(base + i * 4 * 3_600_000).toISOString(),
    }));
}
...
export default function ProjectDetailPage() {
  ...
  async function handleReassign(companySlug: string) {
    ...
    await (supabase.from("projects") as AnyQuery)
      .update({ company_slug: companySlug, updated_at: new Date().toISOString() })
      .eq("id", project.id);
  ...
  function startEdit() {
    ...
    setEditForm({
      ...
      home_type:     project.home_type ?? "",
      description:   project.description ?? "",
      beds:          String(project.beds ?? ""),
      baths:         String(project.baths ?? ""),
      floors:        String(project.floors ?? ""),
      sqft:          String(project.sqft ?? ""),
      base_price:    String(project.base_price ?? ""),
      thumbnail_url: project.thumbnail_url ?? "",
    });
    setThumbPreview(null); // reset so we show editForm.thumbnail_url
    setEditing(true);
  }
  ...
  if (loading) {
    return <div className="flex items-center justify-center h-full"><div className="w-6 h-6 border-2 border-blue-500 border-t-transparent rounded-full animate-spin" /></div>;
  }
  if (!project) {
    return <div className="flex flex-col items-center justify-center h-full gap-3"><p className="text-white/40 text-sm">Project not found.</p><Link href="/admin/requests" className="text-xs text-blue-400 hover:underline">← Back to Queue</Link></div>;
  }
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      ...
                      <div className="col-span-2">
                        ...
                        >
                          {thumbUploading ? (
                            ...
                            // eslint-disable-next-line @next/next/no-img-element
                            <img
                              src={thumbPreview ?? editForm.thumbnail_url}
                              alt="Thumbnail"
                          ...
                          <div className="min-w-0">
                            <p className="text-xs text-white/40">
                              {thumbUploading ? "Uploading…" : "Click or drop to replace thumbnail"}
                            </p>
                      ...
                      <div className="col-span-2">
                        ...
                        <textarea value={editForm.description} onChange={e => setEditForm(f => f && ({ ...f, description: e.target.value }))}
                          rows={3}
                          className="w-full bg-[#111] border border-white/10 rounded-lg px-3 py-2 text-xs text-white/80 focus:outline-none focus:border-blue-500/60 transition-colors resize-none" />
                      </div>
                    </div>
                  </>
                ) : (
                    ...
                    {project.thumbnail_url ? (
                      <>
                        ...
                        <img src={project.thumbnail_url} alt={project.name}
                          className="w-20 h-20 rounded-2xl object-cover mx-auto mb-2 border border-white/10" />
            ...
            <div className="bg-[#1a1a1a] border border-white/8 rounded-xl overflow-hidden">
              ...
              <div className="px-4 py-3 space-y-2.5 border-t border-white/5">
                ...
                <select
                  ...
                  {allBuilders.map(b => (
                    <option key={b.id} value={b.company_slug}>{b.company_name}</option>
                  ))}
                </select>
                {builder && (
                  ...
                  cat.options.filter(o => o.price_impact > 0).length > 0 && (
                    <div key={cat.id} className="flex items-center justify-between">
                      ...
                      <span className="text-xs text-white/40">
                        {cat.options.filter(o => o.price_impact > 0).length} upgrades
                      </span>
                    </div>
                  )
                ))}
