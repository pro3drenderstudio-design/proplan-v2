...
import { Community, Lot } from "@/types/database";
...
export default function CommunityMapPage({
  ...
  params: Promise<{ companySlug: string; communitySlug: string }>;
}) {
  ...
  return (
    <div className="relative h-screen bg-[#080808] overflow-hidden">
            ...
            {imgRect.w > 0 && imgRect.h > 0 && (
              ...
              >
                ...
                {lots.map(lot => {
                  ...
                  return (
                    <g key={lot.id}>
                      {/* Glow layer on hover/select */}
                      {(isHov || isSel) && (
                        <polygon
                          points={pts}
                          fill={s.glow}
                          stroke="none"
                          filter={`url(#glow-${lot.id})`}
      ...
      {/* ── Top bar ──────────────────────────────────────────────────────────── */}
      <div className="absolute top-0 left-0 right-0 z-20 flex items-start justify-between px-5 pt-5 gap-4">

        {/* Builder identity */}
        <div className="flex items-center gap-3" style={{ ...GLASS, borderRadius: 14, padding: "10px 14px" }}>
          {builder?.logo_url ? (
            // eslint-disable-next-line @next/next/no-img-element
            <img src={builder.logo_url} alt={builder.company_name} className="h-7 object-contain max-w-[100px]" />
          ) : (
            <div className="w-8 h-8 rounded-lg bg-blue-600/30 border border-blue-500/30 flex items-center justify-center text-[11px] font-bold text-blue-400">
              <BuilderInitials name={builder?.company_name ?? companySlug} />
            </div>
          )}
          <div className="border-l border-white/10 pl-3">
            <p className="text-[10px] text-white/35 leading-none">Developed by</p>
            <p className="text-xs font-semibold text-white/80 mt-0.5 leading-none">{builder?.company_name ?? companySlug}</p>
            {(builder?.city || builder?.state) && (
              <p className="text-[10px] text-white/25 mt-0.5 leading-none">{[builder.city, builder.state].filter(Boolean).join(", ")}</p>
            )}
          </div>
        </div>
      ...
      {!selectedLot && (
        <div
          className="absolute bottom-5 left-5 z-20 max-w-xs"
        ...
        >
          ...
          {lots.length > 0 && (
            <p className="text-[10px] text-white/20 mt-3 text-center">
              Click a lot on the map to explore
            </p>
          )}
        </div>
      )}
      ...
      >
        {selectedLot && (() => {
          const s = STATUS[selectedLot.status] ?? STATUS.available;
          const proj = selectedLot.project;
          const totalPrice = proj ? proj.base_price + (selectedLot.price_modifier ?? 0) : null;
              ...
              <div className="relative flex-shrink-0">
                {proj?.thumbnail_url ? (
                  <div className="relative h-52 overflow-hidden">
                    ...
                    {/* Lot label over image */}
                    <div className="absolute bottom-0 left-0 right-0 px-5 pb-4">
                      <div className="flex items-end justify-between">
                        <div>
                          <p className="text-[10px] font-bold uppercase tracking-widest text-white/50">Lot</p>
                          <p className="text-2xl font-bold text-white leading-tight">{selectedLot.lot_number}</p>
                        </div>
                        <span
                  ...
                  <div className="flex items-center justify-between px-5 py-4 border-b border-white/8">
                    <div>
                      ...
                      <p className="text-xl font-bold text-white">{selectedLot.lot_number}</p>
                    </div>
                ...
                {proj ? (
                  <div className="px-5 py-4 space-y-5">
                    ...
                    <div>
                      ...
                      {proj.home_type && (
                        <p className="text-xs text-white/35 mt-0.5 capitalize">{proj.home_type.replace("_", " ")}</p>
                      )}
                      {proj.description && (
                        <p className="text-xs text-white/40 mt-2 leading-relaxed">{proj.description}</p>
                      )}
                    </div>
                    ...
                    {totalPrice !== null && (
                      <div className="rounded-xl px-4 py-3.5"
                        ...
                        {(selectedLot.price_modifier ?? 0) !== 0 && (
                          <div className="flex items-center gap-1.5 mt-1.5">
                            <span className="text-[11px] text-white/30">{fmtPrice(proj.base_price)} base</span>
                            <span className="text-[11px] text-white/20">·</span>
                            <span className="text-[11px]" style={{ color: (selectedLot.price_modifier ?? 0) > 0 ? "#fbbf24" : "#34d399" }}>
                              {(selectedLot.price_modifier ?? 0) > 0 ? "+" : "−"}{fmtPrice(Math.abs(selectedLot.price_modifier ?? 0))} lot
                            </span>
                          </div>
                        )}
                      </div>
                    ...
                    {selectedLot.notes && (
                      <div className="rounded-xl px-4 py-3"
                        ...
                        <p className="text-xs text-white/45 leading-relaxed">{selectedLot.notes}</p>
                      </div>
                    )}
                  ...
                  <div className="flex flex-col items-center justify-center py-12 px-5 text-center">
                    ...
                    <p className="text-xs text-white/20 mt-1 leading-relaxed">Contact us to learn more about this lot and available options.</p>
                  </div>
                )}
              </div>
                ...
                <div className="flex-shrink-0 px-5 py-4 border-t border-white/8">
                  ...
                  <div className="space-y-2">
                    {builder.contact_email && (
                      <a href={`mailto:${builder.contact_email}`}
                        className="flex items-center gap-2.5 text-xs text-white/40 hover:text-white/70 transition-colors group">
                        <svg className="w-3.5 h-3.5 text-white/20 group-hover:text-white/40 flex-shrink-0 transition-colors" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.8}>
